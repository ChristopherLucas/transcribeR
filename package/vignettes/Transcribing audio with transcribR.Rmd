...
HP IDOL OnDemand's Speech Recognition API is trained to transcribe both audio files and video files of human speech. It can work with both local files and video files The transcription time of the video depends on the length and the segmentation of the video, and it can take anywhere from 0.05 to 0.20 seconds per second of video.

## API Key

In order to use the Speech Recognition API, one must have an API Key. An API Key can be obtained by making an account at [HP IDOL OnDemand's Website](<https://www.idolondemand.com/signup.html>). API Keys are passed as parameers to the functions `sendAudioGetJobs` and `retrieveText`.
